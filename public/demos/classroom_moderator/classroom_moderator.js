...
var processRequestMessage = function (e) {
  ...
  } else if (e.msg.action === 'start' && e.msg.student === localStream.getID() && question_status !== 'talking') {
...
var set_student_clickable = function (stream) {
  ...
  document.getElementById(`test${id}`).onclick = function (e) {
    const status = room.remoteStreams[id].class_status;
...
var remove_req_icon = function (id) {
  ...
  const el = document.getElementById(`req_img_${id}`);
  if (el != null) {
    el.remove();
  }
