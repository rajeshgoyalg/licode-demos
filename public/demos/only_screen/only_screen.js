...
DEMO.init_demo = function (my_name) {
  const screen = true;
  screen_stream = Erizo.Stream({ screen });
  localStream = Erizo.Stream({
    audio: false, video: false, data: true, screen: false, videoSize: [1024, 640, 1024, 640], attributes: { name: my_name },
  });
...
const add_div_to_grid = function (divId, name) {
...
const remove_div_from_grid = function (divId) {
  const grid = document.getElementById('conference_video_grid');
